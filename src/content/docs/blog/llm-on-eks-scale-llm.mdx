---
title: "LLM on EKS: Scaling with Ray & Karpenter"
date: 2026-07-10
description: Serve Large Language Model on Elastic Kubernetes Service using Virtual Large Language Model with Ray and Karpenter for autoscaling.
excerpt: It's been two months since I deployed my first LLM on Amazon EKS. Now it's time to take the next step—scaling beyond a single GPU using Ray and Karpenter
tags:
  - AWS
  - Kubernetes
  - LLM
...
But after getting it up and running, one question kept coming to mind: _What happens when a single GPU isn't enough?_
...
Together, [Ray](https://github.com/ray-project/ray) and [Karpenter](https://github.com/aws/karpenter-provider-aws) solve different parts of the scaling problem. Ray decides _when_ more workers are needed, while Karpenter makes sure there's actually somewhere for those workers to run.
...
```python title='karpenter_stack.py'
...
But that's also exactly why it's the wrong tool for checking whether autoscaling actually works. A chat UI sends one message, waits for the full response, then sends the next — always one request in flight, never more. `target_ongoing_requests: 2` only kicks in once more requests are _in flight simultaneously_ than a single replica can comfortably handle, and no amount of typing quickly into Streamlit will ever produce that. So to actually see the autoscaler react, 8 requests went out at once via `curl` instead of one at a time through the UI:
...
What's next? I'm really excited about [Kubeflow](https://github.com/kubeflow/kubeflow) — let's see what we can do with it.

Aight. Thanks for reading this one too, hope you found something useful 🚀
