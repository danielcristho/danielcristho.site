---
title: Automating Flask & PostgreSQL Deployment on KVM with Terraform & Ansible
date: 2024-12-25
excerpt: Hi, in this post, we will use Libvirt with Terraform to provision 2 KVM locally and after that, we will Deploy Flask App & PostgreSQL using Ansible.
description: Create VM using Terraform and Deploy Flask Using Ansible.
tags:
  - Ansible
  - Docker
authors:
...
cover:
  alt: Cover image
  image: https://res.cloudinary.com/diunivf9n/image/upload/v1774262924/terraform-ansible_ynyu9s.webp
---
...
```yml title="config/cloud_init.yml"
#cloud-config
runcmd:
...
```yml title="config/network_config.yml"
version: 2
ethernets:
  ens3:
    dhcp4: true
...
```yml title="docker-compose.yml.j2"
version: '3.7'
services:
  postgres:
      image: postgres:13
...
```jinja title="templates/config.py.j2"
...
```yml title="templates/docker-compose.yml.j2"
version: "3.7"
services:
  flask:
    build: flask
...
_NB_: Project Repo: [danielcristho/that-i-write/terrafrom-ansible-flask](https://github.com/danielcristho/that-i-write/tree/main/terrafrom-ansible-flask)
