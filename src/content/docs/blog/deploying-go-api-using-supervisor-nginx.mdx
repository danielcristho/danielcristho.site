...
date: 2025-03-20
description: Deploy a Golang API with Supervisor and serve it through Nginx.
tags:
    - Go
    - Linux
...
![Go, Supervisor & Nginx Cover](https://github.com/user-attachments/assets/6f59eae0-0f46-4dcf-a2d0-e4e079d6ab68)
...
> `directory=/var/www/simple-go-api` → The working directory of the Go API.

> `command=bash -c 'cd /var/www/simple-go-api && ./main'` → Runs the API.
...
> `stdout_logfile=/var/log/api/simple-go-api.out.log` → Standard output log file.

> `stderr_logfile=/var/log/api/simple-go-api.err.log` → Error log file.
