...
export async function getUmamiAuthToken(): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Failed to authenticate with Umami:', response.status, errorText);
      return null;
    }
...
export async function makeUmamiRequest(endpoint: string, options: RequestInit = {}): Promise<Response | null> {
